...
function docReady(){
	...
	var defaultDatableOptions = {
			"sDom": "<'row-fluid'<'span6'l><'span6'fC>r>t<'row-fluid'<'span12'i>C<'span12 center'p>>",
//			"sDom": 'C<"clear">lfrtip',
...
/** autohides columns **/
if (typeof IACUC === 'undefined') {
    var IACUC = {};
  }
    if ($('.iacuc-autohide').length) {
      IACUC.datatableSettings = IACUC.datatableSettings || {};
      defaultDatableOptions.aoColumnDefs = [];
      $('.iacuc-autohide').each(function() {
        var $th = $(this)
        , index = $th.index();
        defaultDatableOptions.aoColumnDefs.push(
          {"bVisible": false, "aTargets": [index]}
        );
      });
    }

	var datatableOptions = $.extend(defaultDatableOptions, IACUC.datatableSettings || {});
		
	//datatable
	window.datatable = $('.datatable').dataTable(datatableOptions);
